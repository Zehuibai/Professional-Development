...
#install.packages("devtools")  # You need to install this package first!
#library(devtools)
...
# Alternatively, try installing it manually: Tools -> Install Packages -> type UBL
library(UBL)
...
# using a relevance function provided by the user 
rel <- matrix(0, ncol = 3, nrow = 0)
rel <- rbind(rel, c(0,   1.0, 0))
rel <- rbind(rel, c(7,   0.5, 0))
rel <- rbind(rel, c(10,  0.0, 0))
rel <- rbind(rel, c(13,  0.5, 0))
rel <- rbind(rel, c(20,  1.0, 0))

plot(rel[,1],rel[,2], ylim=c(-0.2,1.2),type = "l", xlab="Target variable", ylab="Relevance value")
abline(h=c(0.5),lty=3,col="red")

train <- SmoteRegress(score ~ cov1+cov2+cov3+cov4+cov5+cov6+cov7,
                      rel = rel, thr.rel = 0.5,  k = 5,
                      train.org, dist = "Euclidean", C.perc = list(2,1,2))
boxplot(train.org$score)
boxplot(train$score)
...
# Estimation of parameters used for obtaining the relevance function.
control.parms <- phi.control(train[,tgt], method="extremes", extr.type = "both", coef=0.1)
control.parms$control.pts
### Relevance function.
y.phi <- phi(sort(train[,tgt]), control.parms=control.parms)
plot(sort(train[,tgt]), y.phi, ylim=c(0,1), type = "l", xlab = "Target variable", ylab = "Relevance value")
...
impt.ctrl <- rbind(train.org, miss.pred)


impt.ctrl$impt.ubl <- with(impt.ctrl, ifelse(misstype=="MCAR"|misstype=="MAR"|misstype=="MNAR", impt.ctrl$ubl.pred, impt.ctrl$score))
impt.ctrl$impt.rf <- with(impt.ctrl, ifelse(misstype=="MCAR"|misstype=="MAR"|misstype=="MNAR", impt.ctrl$rf.pred, impt.ctrl$score))
impt.ctrl$impt.rf.org <- with(impt.ctrl, ifelse(misstype=="MCAR"|misstype=="MAR"|misstype=="MNAR", impt.ctrl$rf.org.pred, impt.ctrl$score))


summary(impt.ctrl$true)
t.test(impt.ctrl$true, mu = 10, alternative = "two.sided")

summary(train.org$score)
t.test(train.org$score, mu = 10, alternative = "two.sided")

summary(impt.ctrl$impt.ubl)
t.test(impt.ctrl$impt.ubl, mu = 10, alternative = "two.sided")

summary(impt.ctrl$impt.rf)
t.test(impt.ctrl$impt.rf, mu = 10, alternative = "two.sided")

summary(impt.ctrl$impt.rf.org)
t.test(impt.ctrl$impt.rf.org, mu = 10, alternative = "two.sided")
...
impt.all <- rbind(impt.test, impt.ctrl)

lm <- lm(data=impt.all, impt.ubl ~ treat+cov1)
summary(lm)
